...
const FormField = ({ label, id, placeholder, extraClass='', value, onChange, style, setSelectedSource, allFonts, emailValid }) => {
    ...
    return (
        <div id={id} className={`form-field ${extraClass}`}>
            <div className={isFocused ? 'form-field-label label-focused' :'form-field-label'}>
                ...
                {(label === 'email' && isFocused && !emailValid) && <p id="invalid-flag">invalid</p>}
            </div>
